...
void AncestorsNonRecursiveImpl(TreeNode* node, ItemType item);
bool AncestorsRecursiveImpl(TreeNode* node, ItemType item);

void TreeType::Ancestors(ItemType item) {
    AncestorsNonRecursiveImpl(root, item);
}

void TreeType::Ancestors_re(ItemType item) {
    AncestorsRecursiveImpl(root, item);
}

void AncestorsNonRecursiveImpl(TreeNode* node, ItemType item) {
    QueType<TreeNode*> path;
    bool check = false;
...
bool AncestorsRecursiveImpl(TreeNode* node, ItemType item) {
    ...
    if (node == NULL)
        ...
        check = AncestorsRecursiveImpl(node->left, item);
    else
        check = AncestorsRecursiveImpl(node->right, item);
